...
Let $\textbf{X} \in \mathbb{R}_{n \times p}$ be an arbitrary matrix signifying a data frame of $n$ observations in $p$ dimensions. Let $\textbf{M}^{(1)} = \textbf{U}^{(1)}\textbf{DV}^T$ be the \textit{SVD} of an $\textbf{M}$ matrix of $\textbf{X}$. Then by Theorem 1, the linear projection preserving \textit{essentially} all the \textit{BPMC} of $\textbf{X}$ is the first $q_1$ columns of the matrix $\textbf{U}^{(1)}$. Now let $\textbf{M}^{(2)} = \textbf{U}^{(2)}\textbf{DV}^T$ be the \textit{SVD} of the $\textbf{M}$ matrix of $\textbf{X}^{(1)}_{n \times q_1} = \textbf{X}\textbf{U}^{(1)}_{p \times q_1}$. Similarly,
$$
\begin{aligned}
